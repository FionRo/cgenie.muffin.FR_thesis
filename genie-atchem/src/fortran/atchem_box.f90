...
CONTAINS
  ...
  ! ****************************************************************************************************************************** !
  ! OXIDIZE CH4 -- UPDATED PHOTOCHEMICAL SCHEME AFTER GOLDBLATT ET AL. [2006] (CTR|05-2017)
  SUBROUTINE sub_calc_oxidize_CH4_goldblatt(dum_dtyr,dum_conv_atm_mol)
    ...
    ! CH4-O3-O2 photochemistry after Goldblatt et al. [2006] // long-form constants updated from Daines & Lenton [2016]
      ...
      ! *** UPDATE ATM. TRACERS ***
      atm(ia_pCH4,:,:)     = (loc_CH4/loc_atmV)*conv_Pa_atm*const_R_SI*atm(ia_T,:,:)
      atm(ia_pCO2,:,:)     = (loc_CO2/loc_atmV)*conv_Pa_atm*const_R_SI*atm(ia_T,:,:)
      atm(ia_pO2,:,:)      = (loc_O2 /loc_atmV)*conv_Pa_atm*const_R_SI*atm(ia_T,:,:)
      atm(ia_pCH4_13C,:,:) = (loc_13CH4/loc_atmV)*conv_Pa_atm*const_R_SI*atm(ia_T,:,:)
      atm(ia_pCO2_13C,:,:) = (loc_13CO2/loc_atmV)*conv_Pa_atm*const_R_SI*atm(ia_T,:,:)
 ...
 ! ****************************************************************************************************************************** !
 ! OXIDIZE CH4 -- UPDATED PHOTOCHEMICAL SCHEME AFTER CLAIRE ET AL. [2006], NO H ESCAPE (CTR|05-2017)
   SUBROUTINE sub_calc_oxidize_CH4_claire(dum_dtyr,dum_conv_atm_mol)
     ...
     ! *** truncate if beyond training values ***
      ! pO2
      IF (loc_O2 < 1.773E08) THEN
          loc_O2 = 1.773E08
      ELSE IF (loc_O2 > 1.773E19) THEN
          loc_O2 = 1.773E19
      END IF

      ! pCH4
      IF (loc_CH4 < 1.773E14) THEN
          loc_CH4 = 1.773E14
      ELSE IF (loc_CH4 > 3.546E17) THEN
          loc_CH4 = 3.546E17
      END IF
     ...
     loc_dCH4 = min(loc_oxrate*dum_dtyr, 2.0*loc_O2, loc_CH4)
...
! OXIDIZE CH4 -- UPDATED PHOTOCHEMICAL SCHEME AFTER CLAIRE ET AL. [2006], H ESCAPE ENABLED (CTR|05-2017)
  SUBROUTINE sub_calc_oxidize_CH4_claireH(dum_dtyr, dum_conv_atm_mol)
    ...
    REAL:: loc_p00, loc_p10, loc_p01, loc_p20, loc_p11, loc_p02, loc_p30, loc_p21, loc_p12, &
           loc_p03, loc_p40, loc_p31, loc_p22, loc_p13, loc_p04, loc_p50, loc_p41, loc_p32, &
           loc_p23, loc_p14, loc_p05, esc_const
    REAL:: loc_phi_o2, loc_phi_ch4, loc_k
    REAL:: loc_oxrate, loc_dCH4, H_esc, dH_esc
    ...
    ! CH4-O3-O2 photochemistry after Claire et al. [2006] // H escape from the atmosphere enabled
    ! *** truncate if beyond training values ***
     ! pO2
     IF (loc_O2 < 1.773E08) THEN
         loc_O2 = 1.773E08
     ...
     ! *** PERFORM METHANE OXIDATION ***
     ! NOTE: stoichiometry of CH4 oxidation : CH4 + 2O2 + hv  --> CO2 + 2H2O
     !       stoichiometry of H2 escape : CH4 + CO2 + O2 + hv --> 2CO2 + 4H_space
     loc_CH4   = loc_CH4 - loc_dCH4 - dH_esc
     loc_CO2   = loc_CO2 + loc_dCH4 + dH_esc
     loc_O2    = loc_O2  - 2.0*loc_dCH4 - dH_esc
     loc_13CH4 = loc_13CH4 - loc_r13CH4*(loc_dCH4+dH_esc)
     loc_13CO2 = loc_13CO2 + loc_r13CH4*(loc_dCH4+dH_esc)
