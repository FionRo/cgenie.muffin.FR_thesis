        ...
        block_jloop: DO j=1,n_j
           ...
           IF (n_k >= loc_k1) THEN
              ...
              IF (force_restore_ocn_select(io_ALK) .AND. force_flux_ocn_select(io_ALK)) THEN
                 ...
                 ! ------------------------------------------- !
                 IF ( force_restore_atm_select(ia_pCO2) .AND. (.NOT. force_restore_ocn_select(io_colr)) ) THEN
                 ...
                 DO nloc=1,n_orb_pts_nloc 
                    ...
                    DO n=1,n_orb_pts
                       WRITE(unit=out,fmt='(f12.3,999e14.6)',iostat=ios) &
                          & orb_pts_time(n), &
                          & (orb_pts(n_orb_pts,nloc,nvar),nvar=1,n_orb_pts_nvar)
                       call check_iostat(ios,__LINE__,__FILE__)
                    end do
                    CLOSE(unit=out,iostat=ios)
                   call check_iostat(ios,__LINE__,__FILE__)
