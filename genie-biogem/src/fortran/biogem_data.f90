...
CONTAINS
  ...
  SUBROUTINE sub_load_goin_biogem()
    ...
    if (ctrl_debug_init > 0) then
       ...
       print*,'AOM SO4 half sat constant (mol kg-1)                : ',par_bio_remin_AOM_kSO4
       print*,'Thermodynamic drive ID string                       : ',par_bio_remin_AOM_thermo
    ...
    if (abs(par_bio_red_PON_ALK) > const_real_nullsmall) then
       if (ocn_select(io_NO3)) then
          ...
          if (ctrl_bio_red_ALKwithPOC) then
             conv_sed_ocn(io_ALK,is_POC) = par_bio_red_PON_ALK*(1.0/par_bio_red_POP_POC)*par_bio_red_POP_PON
